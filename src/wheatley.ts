...
const channels_map = {
    ...
    // error log
    log: tuple("1260662592746029096", Discord.ThreadChannel),
};
...
export class Wheatley {
    ...
    constructor(
        ...
        this.setup(auth).catch(this.critical_error);
    }
        ...
        this.client.on("ready", () => {
            (async () => {
                ...
                this.client.on("messageCreate", (message: Discord.Message) => {
                    this.on_message(message).catch(this.critical_error);
                });
                this.client.on("interactionCreate", (interaction: Discord.Interaction) => {
                    this.on_interaction(interaction).catch(this.critical_error);
                });
                this.client.on("messageDelete", (message: Discord.Message | Discord.PartialMessage) => {
                    this.on_message_delete(message).catch(this.critical_error);
                });
                this.client.on(
                    "messageUpdate",
                    (
                        ...
                        this.on_message_update(old_message, new_message).catch(this.critical_error);
                    },
                );
                if (!this.freestanding) {
                    await this.populate_caches();
                }
            })().catch(this.critical_error);
        });
